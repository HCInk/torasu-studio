...
void ViewerModule::onBlank(App* instance, const tstudio::blank_callbacks& callbacks) {
	bool destoryTexture = true;
	if (ImageMonitor* imageMon = dynamic_cast<ImageMonitor*>(instance->getMainMonitor()->getMonitorImplementation())) {
...
void ViewerModule::render(App* instance) {
	...
	} else if (NumberMonitor* numMon = dynamic_cast<NumberMonitor*>(instance->getMainMonitor()->getMonitorImplementation())) {
